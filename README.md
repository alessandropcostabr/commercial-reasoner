...
(`src/commercial_reasoner/memory/gate.py` + `numeric_guard.py`) on the `/reason` service
path (the path LATE integrates with, see `API_CONTRACT_LATE.md`): every reply the reasoner
produces through `/reason` is scanned before it goes out. Replies generated by a gateway
that does not route through `/reason` are outside this gate's coverage. What the scan does:
